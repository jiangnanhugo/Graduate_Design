...
长距离短期记忆网络是根据RNN所改进的最著名的模型之一。LSTM的计算公式定于如下\upcite{DBLP:journals/neco/HochreiterS97}： 1）输入门$i_t$。控制当前输入 $x_t$ 和前一步输出 $h_{t−1}$ 进入新的 cell 的信息量；2） 遗忘门$f_t$ 。决定是否清除或者保持单一部分的状态；3） 变换输出和前一状态到最新状态~$g_t$；4） 输出门$o_t$。计算 cell 的输出 ；5） cell 状态更新$s_t$。其计算步骤是，计算下一个时间戳的状态使用经过门处理的前一状态和输入；6） 最终 LSTM 的输出$h_t$。使用一个对当前状态进行 $\tanh$ 变换。其计算公式如下：
