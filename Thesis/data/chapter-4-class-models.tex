...
\KwData{ 隐藏层输出 $h$}
 \For{$c \in \mathcal{C}$ }{
  {Calculate $\log p^o(w| c,h)$} \tcp*[r]{计算类别的概率}
 \For{w $\in$ c}{
 {Calculate $\log p^w(\hat y^w|\hat y^c,h)$}\;
  {$\log p(w|h)=\log p^c(y^c|h)+\log p^w(\hat y^w|\hat y^c,h)$}\tcp*[r]{计算每个单词全局概率}
 }
 }
...
\KwData{ 隐藏层输出 $h$}
 {$\hat y^o=\arg\max_o{\log p^o(w| c,h)}$ }\tcp*[r]{挑选每个类中概率最大的单词}
 {// 在这些已经被栅选单词中挑选最佳的单词}
