...
\chapter{并行树状概率计算模型}
\section{引言}
在本章中，我们将这个预测端的词表用二叉树的分层的形式表示，使单词基于二叉树的路径的编码适配基于树的分层概率计算。首先，我们提出了一个在分层结构上建模参数的单词编码方案。与此同时，因为考虑到GPU设备上的算法的并行计算的吞吐性能，我们进而推导出紧凑的代价函数及其梯度公式，使得该分层模型在GPU设备上的能并行计算分层模型的内部概率函数，而不是线性计算模型概率，浪费GPU设备的高并行的特性。
...
\begin{figure}[!h]
  \centering
    \includegraphics[width=1\linewidth]{./figures/thsm.pdf}
...
\begin{figure}[!ht]
  \centering
\includegraphics[width=1\linewidth]{./figures/relus.pdf}
...
1）一元单词聚类算法。它根据词频对词汇进行排序，并根据单词统计从底端到顶端进行合并，也称为霍夫曼编码~\upcite{DBLP:conf/nips/MikolovSCCD13}。
...
当我们构造完霍夫曼二叉树后，我们需要遍历这棵树来获取所有单词的极性编码霍夫曼编码：
\begin{enumerate}
